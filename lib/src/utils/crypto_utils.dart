...
class CryptoUtils {
  ...
  static AsymmetricKeyPair<RSAPublicKey, RSAPrivateKey> generateRsaKeyPair({int bitLength = 4096}) {
    final keyGenerator = RSAKeyGenerator()
      ..init(ParametersWithRandom(RSAKeyGeneratorParameters(BigInt.from(65537), bitLength, 64), _getSecureRandom()));
  ...
  static Future<AsymmetricKeyPair<RSAPublicKey, RSAPrivateKey>> generateRsaKeyPairAsync({int bitLength = 4096}) {
  ...
  /// * SHA-512/RSA
  ///
  static Uint8List rsaSign(RSAPrivateKey privateKey, Uint8List data, {String algorithmName = 'SHA-256/RSA'}) {
  ...
  static RSAPrivateKey decodeRsaPrivateKeyFromPkcs8(Uint8List bytes) {
    ...
    final rsaPrivateKey = RSAPrivateKey(modulus.integer!, privateExponent.integer!, p.integer, q.integer);
  ...
  ///
  static ECPrivateKey decodeEcPrivateKey(Uint8List bytes, {bool pkcs8 = false, ECDomainParameters? parameters}) {
    ASN1Parser asn1Parser = ASN1Parser(bytes);
    final topLevelSeq = asn1Parser.nextObject() as ASN1Sequence;
    String curveName;
    Uint8List x;
    ...
    return ECPrivateKey(osp2i(x), parameters ?? ECDomainParameters(curveName));
  }
