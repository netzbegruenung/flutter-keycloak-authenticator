...
import 'package:flutter_keycloak_authenticator/src/authenticator.dart';
import 'package:flutter_keycloak_authenticator/src/dtos/challenge.dart';
import 'package:flutter_keycloak_authenticator/src/enums/key_algorithm_enum.dart';
import 'package:flutter_keycloak_authenticator/src/enums/signature_algorithm_enum.dart';
import 'package:flutter_keycloak_authenticator/src/keycloak_client.dart';
import 'package:flutter_keycloak_authenticator/src/utils/crypto_utils.dart';
...
class KeycloakAuthenticator implements Authenticator {
  ...
  KeycloakAuthenticator._({required _Data data, required KeycloakClient client}) : _data = data, _client = client;
  ...
  Future<void> updateDevicePushId({required String? devicePushId}) async {
    await _client.updateDevicePushId(deviceId: _data.id, devicePushId: devicePushId);
  }
}
...
class _Data {
  ...
  static PrivateKey _decodePrivateKey(KeyAlgorithm keyAlgorithm, String encodedKey) {
    switch (keyAlgorithm) {
      case KeyAlgorithm.RSA:
        return CryptoUtils.decodeRsaPrivateKeyFromPkcs8(base64Decode(encodedKey));
      case KeyAlgorithm.EC:
        return CryptoUtils.decodeEcPrivateKey(base64Decode(encodedKey), pkcs8: false);
    }
  }

  static String _encodePrivateKey(KeyAlgorithm keyAlgorithm, PrivateKey privateKey) {
    switch (keyAlgorithm) {
      case KeyAlgorithm.RSA:
  ...
  factory _Data.fromJson(Map<String, dynamic> json) {
    ...
    return _Data(
      ...
      registeredAt: DateTime.fromMillisecondsSinceEpoch(json['registeredAt'] as int, isUtc: true),
    );
  }
