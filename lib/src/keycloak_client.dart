...
import 'package:flutter_keycloak_authenticator/src/exceptions/keycloak_client_exception.dart';
import 'package:flutter_keycloak_authenticator/src/utils/crypto_utils.dart';
import 'package:flutter_keycloak_authenticator/src/utils/device_utils.dart';
...
class KeycloakClient {
  ...
  String buildSignatureHeader(String keyId, Map<String, String> keyValues) {
  ...
  Future<List<Challenge>> _getChallengesRequest(String deviceId) async {
    final signatureHeader = buildSignatureHeader(deviceId, {
      'created': (DateTime.now().millisecondsSinceEpoch - 1000).toString(),
    });
    final res = await _dio.get<List<dynamic>>(
      '/challenges',
      queryParameters: {'device_id': deviceId},
      options: Options(headers: {'signature': signatureHeader}),
    );
  ...
  Future<void> _challengeReplyRequest(
    ...
    await _dio.get<void>(
      '/login-actions/action-token',
      queryParameters: {'client_id': clientId, 'tab_id': tabId, 'key': key, 'granted': granted},
      options: Options(headers: {'signature': signatureHeader}),
    );
  }

  Future<void> updateDevicePushId({required String deviceId, required String? devicePushId}) async =>
